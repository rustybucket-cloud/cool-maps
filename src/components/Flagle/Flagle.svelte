<script>
  ...
  function judgeGuess(e) {
    ...
    const url = `/api/flagle?guess=${guessVal}&country=${country}`
    loading = true
    fetch(url)
      .then((req) => req.json())
      .then((data) => {
        const distance = data.distance
        distances[guess] = Math.round(100 - (distance * 100))
  ...
  function getDistanceText() {
    const distance = distances.reduce((prev, curr) => prev + (100 - curr), 0)
    ...
    <div class="flex gap-4">
      ...
      {#if distance}
        <div>
          <p>{distance}%</p>
          <div>
            <div class="h-2" style={`width: ${distance}%; background-color: rgb(${255 - (255 * (distance * 0.01))}, ${255 * (distance * 0.01)}, 0);`} />
          </div>
        </div>
      {/if}
    </div>
...
{#if loading || isCorrect || gameOver}
  <div class="w-screen h-screen absolute inset-0 flex justify-center items-center wrapper z-50">
